...
function paintSign(ctx: Ctx, y0: number, text: string, colors: [string, string], font: SignFont) {
  const h = 64;
...
function buildTexture(): THREE.DataArrayTexture {
  const signs: { id: string; text: string; colors: [string, string]; font: SignFont }[] = [
  ...
  for (let s = 0; s < signLayers; s++) {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      if (!sg) break;
      paintSign(ctx, k * 64, sg.text, sg.colors, sg.font);
