...
Additional final views: [NorCal day](qa/norcal-after-day-40.jpg), [Florida night](qa/florida-after-night-40.jpg), [signalized street by day](qa/street-signal-day.jpg), [signalized street at night](qa/street-signal-night.jpg), [hill junction](qa/street-hill-junction.jpg), and [commune remains after departure](qa/commune-burnt-remains.jpg).
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| High, 1280 × 800 | 221 buildings, 8,568 people | 75.1 | 13.3 ms | 7.8 ms | 147 | 3,398,894 | 100% |
| Low, 390 × 844 | 218 buildings, 7,874 people | 131.3 | 7.6 ms | 5.9 ms | 112 | 1,223,972 | 100% |
...
2. Ground-detail placement previously took 3.4 ms median and up to 9.3 ms in a dense town. The incremental refill now measured **0.6 ms median**, with a **0.9 ms maximum** across the final High and Low town runs.
...
4. The building geometry kit now shares quad corners. The final High town uses **202,284 stored building vertices** (221 buildings) and the Low town uses **192,700** (218 buildings), with the same rendered triangles and visual detail. A deterministic 10,800-building generation matrix measured a 32.8% drop in stored vertices.

An interleaved GPU timer profile measured about 7.55 ms for a rendered High frame at 60% scale. The largest remaining GPU costs in that view were trees (2.57 ms), the complete post pass (2.49 ms; AO alone was 0.08 ms), and shadows (1.52 ms). A contended concurrent-browser run briefly measured 47.9 FPS and scaled to 60%; exclusive hardware runs consistently exceeded 70 FPS at 100% scale. The final run above includes the latest hill-road grading and gameplay hooks from the base branch.

`scripts/touchtest.mjs` passed at 390 × 844 on Low after the latest base merge: the drag created four road segments, Done ended drawing, and Undo restored the original segment count. The departure test confirmed `gone` state, attached burnt remains, scorched terrain paint, and no browser errors. The final 36-view commune matrix across all three maps, plus the latest street and departure captures, had no browser or shader errors. A steep hill junction exposed an approach-height seam, now fixed by matching the junction hull edge to sampled road heights. A fresh-storage desktop boot auto-selected Ultra without reloading or errors; the F3 overlay kept updating. Cold High boots in the atmosphere lab on Appalachia and Florida had no WebGL, shader, or program-link errors after correcting reflection render order. TypeScript and the single-file build passed before the branch push.
