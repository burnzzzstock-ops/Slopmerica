// Sky dome, sun + moon, clouds, stars + Milky Way, height fog and the day/night
// cycle. WeatherSystem writes `env.wx` and `env.dayOfYear` every frame; the sky
// never reads the weather system directly, so it also works without one.
...
/** Weather inputs, written by WeatherSystem each frame. */
export interface SkyWeather {
  cloudCover: number; // 0..1 clouds in the sky (and their ground shadows)
  overcast: number; // 0..1 flat grey light
  precip: number; // 0..1 rain/snow darkening
  haze: number; // distance haze multiplier (1 = the map's normal haze)
  mist: number; // ground mist amount 0..8
  mistHeight: number; // meters
  smoke: number; // 0..1 wildfire smoke
  storm: number; // 0..1 hurricane green-black
  heat: number; // 0..1 heatwave glare
  flash: number; // 0..1 lightning
  snowCover: number; // 0..1 ground snow (brighter bounce light)
  cloudSpeed: number; // cloud drift, m/s
}
...
export class Environment {
  ...
  day = 0;
  /** Override the moon phase (0 new, 0.5 full); null = follow the calendar. */
  moonPhaseOverride: number | null = null;
  /** Weather inputs (see SkyWeather). */
  readonly wx: SkyWeather = { cloudCover: 0.22, overcast: 0, precip: 0, haze: 1, mist: 0, mistHeight: 30, smoke: 0, storm: 0, heat: 0, flash: 0, snowCover: 0, cloudSpeed: 6 };

  // outputs other systems read (water, weather, particles)
  ...
  /** Rough overall scene light level 0..1 (for unlit shaders). */
  lightLevel = 1;
  ...
  constructor(private scene: THREE.Scene, private def: MapDef, q: Quality) {
    ...
    const warm = new THREE.Color(0xffd2a8), cool = new THREE.Color(0xb8ccff), c = new THREE.Color();
    for (let i = 0; i < n; i++) {
      ...
      seed[i] = rnd();
      c.copy(cool).lerp(warm, rnd()).lerp(new THREE.Color(1, 1, 1), 0.45);
  ...
  update(dtHours: number, focus: THREE.Vector3, viewDist: number) {
    ...
    sampleKeys(GLOW, elev, glow).multiplyScalar(clear * (1 - wx.smoke * 0.5) * 0.9);
    ...
    const dim = 1 - wx.precip * 0.45 - wx.storm * 0.35;
    this.zenith.lerp(this.tmp.copy(C_OVERCAST).multiplyScalar(lumH * 0.95 * dim), wx.overcast);
    this.horizon.lerp(this.tmp.copy(C_OVERCAST).multiplyScalar(lumH * 1.02 * dim), wx.overcast);
    if (wx.storm > 0) {
      const day = 0.15 + daylight * 0.85;
      this.zenith.lerp(this.tmp.copy(C_STORM_Z).multiplyScalar(day), wx.storm);
      this.horizon.lerp(this.tmp.copy(C_STORM_H).multiplyScalar(day * 0.7), wx.storm);
    }
    if (wx.smoke > 0) {
      const day = 0.12 + daylight * 0.88;
      this.zenith.lerp(this.tmp.copy(C_SMOKE_Z).multiplyScalar(day * 0.9), wx.smoke);
      this.horizon.lerp(this.tmp.copy(C_SMOKE_H).multiplyScalar(day * 0.8), wx.smoke);
    }
    if (wx.heat > 0) {
      // bleached, baking sky: cream haze at the horizon, washed-out blue overhead
      this.horizon.lerp(this.tmp.copy(C_HEAT_H).multiplyScalar(Math.max(lumH, 0.2) * 1.2), wx.heat * 0.7);
      this.zenith.lerp(this.tmp.copy(C_HEAT_Z).multiplyScalar(Math.max(lumH, 0.2) * 1.1), wx.heat * 0.45);
    }
    if (wx.mist > 0) this.horizon.lerp(this.tmp.copy(C_MIST).multiplyScalar(Math.max(lumH, 0.05)), Math.min(1, wx.mist * 0.2));

    // ---- sun light
    sampleKeys(SUNCOL, elev, this.sunColor);
    if (wx.smoke > 0) this.sunColor.lerp(C_SMOKE_SUN, wx.smoke * 0.75);
    if (wx.heat > 0) this.sunColor.lerp(this.tmp.setRGB(1, 0.8, 0.55), wx.heat * 0.35);
    let sunI = sampleScalar(SUN_I, elev);
    sunI *= 1 - wx.overcast * 0.82;
    sunI *= 1 - wx.smoke * 0.5;
    sunI *= 1 - wx.storm * 0.6;
    sunI *= 1 + wx.heat * 0.12;
    ...
    const moonI = (0.2 + 0.72 * illum) * moonUp * THREE.MathUtils.smoothstep(-elev, 3, 9) * (1 - wx.overcast * 0.75);
    ...
    this.sun.position.copy(focus).addScaledVector(this.lightDir, 1500);
    ...
    hemiI *= 1 + wx.overcast * 0.3 - wx.storm * 0.35 - wx.smoke * 0.15 - wx.precip * 0.15;
    ...
    this.hemi.groundColor.copy(this.hemiGround).lerp(C_SNOW_GROUND, wx.snowCover * 0.6).multiplyScalar(0.4 + daylight * 0.6);
    if (wx.flash > 0) {
      this.hemi.color.lerp(C_FLASH, Math.min(1, wx.flash));
      hemiI += wx.flash * 3.5;
    }
    ...
    this.fog.density = this.def.sky.fogDensity * wx.haze;
    this.fog.mist = wx.mist;
    this.fog.mistHeight = wx.mistHeight;
    this.fog.pack();
    ...
    const sunVis = THREE.MathUtils.smoothstep(elev, -2, 1) * (1 - wx.overcast * 0.97) * (1 - wx.storm * 0.9);
    u.uSunVis.value = sunVis;
    (u.uSunColor.value as THREE.Color).copy(this.sunColor).multiplyScalar(1 + wx.smoke * 0.5);
    u.uSunSize.value = wx.smoke > 0.3 ? 0.016 : 0.012;
    u.uMoonPhase.value = phase;
    u.uMoonBright.value = (0.6 + 2.2 * this.night) * moonUp * (1 - wx.overcast * 0.9) * (1 - wx.smoke * 0.6);
    const starVis = THREE.MathUtils.smoothstep(this.night, 0.55, 1) * (1 - wx.cloudCover) ** 2 * (1 - wx.smoke) * (1 - Math.min(1, wx.mist * 0.15));
    u.uStars.value = starVis * (1 - lp) ** 2;
    u.uCloudCover.value = wx.cloudCover;
    u.uCloudDark.value = Math.min(1, wx.precip * 0.7 + wx.storm);
    u.uFlash.value = wx.flash;
    u.uHorizonFog.value = 0.05 + Math.min(0.25, wx.mist * 0.03 + wx.overcast * 0.04 + wx.smoke * 0.1);
    // clouds are lit by the sun by day and by the city/moon at night
    const cl = u.uCloudLit.value as THREE.Color;
    const cs = u.uCloudShade.value as THREE.Color;
    cl.copy(this.sunColor).multiplyScalar((0.25 + sunI * 0.55) * daylight).lerp(this.horizon, 0.25 + wx.overcast * 0.5 + this.night * 0.5);
    cs.copy(this.zenith).lerp(this.horizon, 0.5).multiplyScalar(0.62);
    if (this.night > 0) {
      addScaled(cl, C_LP, lp * this.night * 0.12);
      addScaled(cs, C_LP, lp * this.night * 0.06);
      addScaled(cl, C_MOON, moonI * this.night * 0.12);
    }
    this.cloudOff.x += wx.cloudSpeed * dt * 0.0005 * atmo.uWindDir.value.x;
    this.cloudOff.y += wx.cloudSpeed * dt * 0.0005 * atmo.uWindDir.value.y;
    (u.uCloudOffset.value as THREE.Vector2).copy(this.cloudOff);
