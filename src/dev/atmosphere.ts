// Atmosphere Lab: builds the world the way src/game.ts does (terrain, trees,
// water, env) and wires in WeatherSystem, PostFX, Particles and AudioEngine.
...
// Scripting: window.__atmo.set({ day, hour, weather, ... }) for screenshots.
import '../style.css';
import * as THREE from 'three';
import { defaultQuality, QUALITY } from '../config';
import { generateMap, MAPS, type MapId } from '../world/maps';
...
const START: Record<MapId, { x: number; z: number; yaw: number }> = {
  appalachia: { x: -120, z: 60, yaw: 0.7 },
  norcal: { x: -380, z: 120, yaw: -1.9 },
  florida: { x: 0, z: 200, yaw: 3.0 },
};
...
class Lab {
  ...
  constructor(container: HTMLElement) {
    ...
    this.water = createWater(this.terrain, map.def.water);
    this.scene.add(this.water.mesh);
    this.trees = new Trees(this.terrain, map, this.q.treeDensity);
    this.scene.add(this.trees.group);
    this.env = new Environment(this.scene, map.def, this.q);
    ...
    this.weather = new WeatherSystem({
      scene: this.scene, renderer: this.renderer, env: this.env, terrain: this.terrain, trees: this.trees, water: this.water,
      quality: this.q, mapId: this.mapId, post: this.post, particles: this.particles, audio: this.audio,
    });
  ...
  setCam(name: string) {
    ...
    const presets: Record<string, [number, number]> = { start: [820, 0.72], hero: [430, 0.2], close: [170, 0.42], far: [1900, 0.95], top: [1300, 1.42] };
  ...
  set(o: { day?: number; hour?: number; weather?: WeatherKind | 'auto'; cam?: string; lp?: number; moon?: number | null; tilt?: boolean; city?: number; play?: boolean }) {
    ...
    if (o.weather) {
      if (o.weather === 'auto') this.weather.force('clear', 0.01);
  ...
  frame(dt: number) {
    ...
    if (this.fx) {
      ...
      if (this.fxTimer <= 0) {
        ...
        this.particles.emit(this.fx, p.x, p.y + (this.fx === 'cigarette' ? 1.6 : 1), p.z);
