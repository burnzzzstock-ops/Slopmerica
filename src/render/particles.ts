...
export class Particles {
  /** World wind (m/s) that pushes smoke, steam and paper. WeatherSystem sets it when linked. */
  readonly wind = new THREE.Vector3(0.6, 0, 0.3);
  /** Scene light level 0..~1.5 for unlit puffs (WeatherSystem sets it when linked). */
  light = 1;
  private normal: Pool;
  ...
  constructor(scene: THREE.Scene, q: Quality) {
    ...
    const mk = (blending: THREE.Blending) => {
      ...
      uniforms.uWind = { value: this.wind };
